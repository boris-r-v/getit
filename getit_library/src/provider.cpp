...
getit::Provider::Provider( boost::program_options::variables_map const& vm, boost::asio::io_service& io ):
    timer_ ( io )
{
    std::cout << "getif::Provider ctor: " << vm["indir"].as<std::string>() << " " << vm["outdir"].as<std::string>() << std::endl;
}
